// Package hsl provides functions for creating colors from an OK HSL profile.
package uc
...
func HSL(h, s, l float64) Color { // Color.from_ok_hsl
	if l == 1.0 {
	...
	return NewColor(
		...
		1,
	).Clamp(Color{0, 0, 0, 0}, Color{1, 1, 1, 1})
}
...
func HSLA(h, s, l, a float64) Color { // Color.from_ok_hsl
	c := HSL(h, s, l)
	c[3] = float32(a)
	return c
}
